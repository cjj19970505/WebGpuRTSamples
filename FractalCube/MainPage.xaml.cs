...
namespace FractalCube
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        GpuTexture CubeTexture { get; set; }
        GpuSampler Sampler { get; set; }
        GpuBindGroupLayout UniformBindGroupLayout { get; set; }
        public MainPage()
        {
        ...
        void DrawFrame()
        {
            ...
            GpuRenderPassDescriptor renderPassDescriptor = new GpuRenderPassDescriptor(new GpuRenderPassColorAttachment[] { new GpuRenderPassColorAttachment(swapChainTexture.CreateView(), new GpuColorDict { R = 0.5f, G = 0.5f, B = 0.5f, A = 1.0f }) })
            {
            ...
            passEncoder.EndPass();
            commandEncoder.CopyTextureToTexture(new GpuImageCopyTexture(swapChainTexture), new GpuImageCopyTexture(CubeTexture), new GpuExtend3DDict { Width = SwapChainDescriptor.Width, Height = SwapChainDescriptor.Height, Depth = 1 });
        ...
        private async void Page_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            while (true)
            {
                if (SwapChain == null)
                {
                    ...
                    CubeTexture = Device.CreateTexture(new GpuTextureDescriptor(new GpuExtend3DDict { Width = SwapChainDescriptor.Width, Height = SwapChainDescriptor.Height, Depth = 1 }, GpuTextureFormat.BGRA8UNorm, GpuTextureUsageFlags.Sampled | GpuTextureUsageFlags.CopyDst));
